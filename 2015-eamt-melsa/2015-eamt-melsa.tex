...
selections.\todo{METIS too?}  Nowadays, in state-of-the-art phrase-based statistical
...
generation. The relevant \todo{OK?}  modules of the system for this method are the \emph{lexical transfer}
...
--- although for all pairs there is at least \todo{check "at least" here} a parallel corpus available, which
...
%  system. 
  ...
  default.\footnote{A bilingual dictionary in Apertium
    ...
    and marks it as the \emph{linguistic default} translation.}\todo{Doesn't this happen in br--fr too?} As a
...
%    (it mentions the use of TL info for lexical selection.},
  ...
  frequency.\todo{give SVN rev. for all?}\todo{Default is explained for some LPs only, why?} 
...
overview of the inputs. \todo{Could we add "diff()" and "amb()" if we had some space?} In the description it is assumed that the
...
\item \textbf{South-East European Times} (\texttt{SETimes}): Described
  ...
  portion will be used.\todo{Not in OPUS?}
\item \textbf{Open Data Euskadi} (\texttt{OpenData}): This is a Basque
  ...
  of Public
  Administration'.\footnote{\url{http://tinyurl.com/eu-es-tm}}\todo{shortened URL}
...
development. \todo{Why do ambiguity numbers change so much from the paper? Having devel and test in different columns would make the caption smaller and the table would still fit}%Table~\ref{table:test-corp} gives information about the
%test corpora.
\todo{``av. amb'' is a corpus average or a dictionary average?}
