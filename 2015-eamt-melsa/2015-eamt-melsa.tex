...
% \todomlf{R1 asked for this, even if redundant, and says: ``can one source word be translated as multiple words? What about multi-word expressions that cannot be translated word-by-word or source words that do not have a TL equivalent'', etc.} % and \(T_s(\cdot)\)
%is the set of all
...
overview of the inputs.%\todomlf{Caption of figure extended in response to R3} 
...
%\todomlf{BLEU reference advanced, and wrong section link removed} 
...
to calculate the feature weights.  %\todomlf{R3 says: ``The proposed
  ...
  % something in the discussion (future work).}
After learning the
