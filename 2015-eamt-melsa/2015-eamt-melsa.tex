...
%  taking into account all of the lexical selection possibilities.
...
\comment{This section should be ironed out a bit.}  In their
...
word form,\comment{[:we do too, integrate this later]} using SL context to
...
system.\comment{[how is our work different?]}  In their approach, a
...
\comment{Reword this to avoid repetition} The method used here
...
word \emph{arrain} into Spanish.\footnote{The word \emph{arrain} `fish' has two translations
...
\item[Basque--Spanish:] The development of the Basque to Spanish pair
  ...
  dictionary.% , but non-default translations were marked with a
...
translation is selected. \comment{FSM: How many times does this
  happens?. I would move this last sentence to an appendix.}  The
...
\comment{This part of the text needs compressing and merging} This is
...
due to inadequate transfer of morphological features. For example, if
...
--- may also lead to the same problem.  The {\sc bleu} metric \citep{papineni02} is
included only as it is commonly used to evaluate MT systems. Other
...
\begin{itemize}
\item \textbf{Linguist-chosen defaults}. \comment{FSM: This should go
    to the section explaining Apertium.}  A bilingual dictionary in an
  ...
  the most general, it is marked as the default by the linguist who is
...
\item \textbf{Target language model} (TLM). One method of lexical
  ...
  \cite{melero07a}.  We used a five-gram language model of surface
...
%tables~\ref{table:eval-para-results} and \ref{table:eval-mono-results}.
 ...
 computational cost.\comment{FSM: This needs to be proved, and for
...
Improvements with
 ...
 contexts across the whole training corpus. \comment{First attempt at
