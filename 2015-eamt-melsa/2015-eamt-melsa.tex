...
translation; they used a parser to identify syntactic relations
...
dependency-grammar-based transfer system TectoMT using a bilingual corpus.
More recently, \cite{tyers12a} presented a method of lexical selection for rule-based
...
\section{Method}
\todomlf{R3 says: ``The technique clearly resembles the "Deciphering Foreign Language," (S. Ravi and K. Knight) or “EM Decipherment for Large Vocabularies “
(M. Nuhn, and H. Ney. EM) approaches, although these techniques were not meant for learning the disambiguation but for learning the translation dictionaries.
The proposed method only executes one EM iteration in contrast to these works. It would be interesting to highlight differences and resemblances with these deciphering approaches.''. Check and cite? Also, single iteration WTF?  }
...
word $s$ in a source-language context $c$ be $p_s(t|c)$. In principle, this value
...
done when $(s, t, c)$ is not found in the corpus?
A maximum-entropy model answers both of these questions. It allows the
...
is a translation, and $c$ is a source-language (SL) context.\todomlf{Defined SL which had not been defined} One difference is that \cite{berger1996} take $s$, $t$
and $c$ to be based on word forms, whereas in our method they are based
on lemma forms.
An example would be the following feature where the Spanish word \emph{pez} (`fish' as a living animal) is seen as the
...
\end{equation}
\todomlf{R3 complains: `` Probability notation is confusing. I would suggest the use of \(p(t | c,s)\) instead of \(p_s(t|c)\), that will simplify notation. For instance instead of \(Z^s (c)\) we would have \(Z(c,s)\).''. Ignore or take onboard?}
...
\todomlf{R3 says: ``it would have been nicer to see an example of one sentence going through the pipeline''. Can we add this in the text?}
...
suitable normalisation:\todomlf{R2 says ``Nevertheless, I've got problems to understand (4), in particular the paths in post-lexsel step. Some better explanation could be offered maybe by saying that (4) evaluates all possible solutions in the language model.''. Don't know what to do here. }
\begin{equation}
...
number $p(g_i|S)$ of times. We prune $(s, t, c)$
...
\todomlf{R1 says ``whole section is rather length especially the revision numbers and URLs of the specific translations systems need not be mentioned in my opinion. The space could rather be used to elaborate in the description of the method in Sec 2.''}
...
%  frequency. \todo{Default is explained for some LPs only, why?}  
  ...
  41387.\footnote{\url{https://svn.code.sf.net/p/apertium/svn/trunk/apertium-en-es}}\todomlf{4-month mention removed in response to R1}
\end{description}
...
the lexical selection module $T_t(s_i)$ is empty.  Recall that,\todomlf{R1 says ``this has already been mentioned before''. Don't know what to do}
...
as each ambiguous word has a \emph{default} translation.}\todomlf{R1 complains: ``why mention if it doesn't happen in practice? Relevance?'' What should we do?}
...
\todomlf{R3 says: ``The proposed maximum entropy or log-linear model is trained in a surprising way. In the early days of maximum entropy models, those were trained using the GIS algorithm. However, it has been proved in the literature that GIS has clear disadvantages with respect to other batch gradient descent algorithms. For instance, it does not allow for the use of regularization [...], and it is extremely slow to converge, which yields worse models than other methods. The smoothing/regularization of the model is also poor since it is simply feature pruning instead of regularization.''}
After learning the feature sets and
weights, we compute the evaluation measures described in
Section~\ref{ss:measures}. There is an option to remove events 
$(s, t, c)$ which occur
...
%tables~\ref{table:eval-para-results} and \ref{table:eval-mono-results}.
 ...
 linguist-chosen defaults),\todomlf{R1 says the footnote is important and should be incorporated to the text; what should we do?}\footnote{The high error rate
 ...
 to the target-language model performance in terms of similar or better BLEU and LER scores, at a much smaller
 computational cost.
...
selections for each sentence using the target-language model online.\todomlf{R2 says: ``An error analysis would also help to understand the proposal.'' We don't have time now I think.}
