...
\todomlf{R1 has a general comment: ``watch out for long sentences''}
...
refer to what is actually target word selection\todofsm{Should we call
  it lexical selection instead of target word selection?} in MT; they
...
\section{Method}
\todomlf{R3 says: ``The technique clearly resembles the "Deciphering
  Foreign Language," (S. Ravi and K. Knight) or “EM Decipherment for
  Large Vocabularies “ (M. Nuhn, and H. Ney. EM) approaches, although
  these techniques were not meant for learning the disambiguation but
  for learning the translation dictionaries.  The proposed method only
  executes one EM iteration in contrast to these works. It would be
  interesting to highlight differences and resemblances with these
  deciphering approaches.''. Check and cite? Also, single iteration
  WTF?}
...
maximum-entropy model maximise the entropy subject to match the
expected counts of the designed features with those found in the
training data. \todofsm{Solved R3: ``In answer to the second question, a
  maximum-entropy model assumes nothing about what is not present in
  the training data. That is, if there is no information in the
  training data, then it assumes that all outcomes —that is, all
  possible translations— are equally likely'' can be applied to a
  uniform distribution for a discrete variable. I think the authors
  are trying to restate the main property of the maximum entropy
  model, i.e.  maximum entropy model maximize the entropy (which is
  the uniformness of the distribution) subject to match the expected
  counts of the designed features with those found in the training
  data.  }
%assumes nothing about what is not present in the
%training data. 
...
$h_k^{s}(t, c)$, where $t$ is a translation, and $c$ is a SL
...
problem word and one word (+ \emph{handi}) to the right of it.\todomlf{Removed \(k\) and used a more informative subscript}
...
weight $\lambda_k^s$. \todomlf{Reworded to define \(n_F\)} Combining these weights of active features
...
\end{equation}
\todomlf{R3 complains: `` Probability notation is confusing. I would
  suggest the use of \(p(t | c,s)\) instead of \(p_s(t|c)\), that will
  simplify notation. For instance instead of \(Z^s (c)\) we would have
  \(Z(c,s)\).''. Ignore or take onboard? FSM: Mention that we follow
  the notation of Berger?}
...
choices of those SL words, where \(t_i\) is an element of \(T_s(s_i)\), the set of possible translations of \(s_i\).\todomlf{R1 asked for this, even if redundant, and says: ``can one source word be translated as multiple words? What about multi-word expressions that cannot be translated word-by-word or source words that do not have a TL equivalent'', etc.} % and \(T_s(\cdot)\)
%is the set of all
...
\todomlf{R3 says: ``it would have been nicer to see an example of one
  sentence going through the pipeline''. Can we add this in the text?
  FSM: We do not have much space}
...
suitable normalisation:\todomlf{R2 says ``Nevertheless, I've got
  problems to understand (4), in particular the paths in post-lexsel
  step. Some better explanation could be offered maybe by saying that
  (4) evaluates all possible solutions in the language model.''. Don't
...
\todomlf{R1 complains: ``the actual maximum entropy method itself is described very briefly (3 sentences at the end of Section 2)''}
...
SL \emph{lexical forms} --- lemmas and  % combinations of lemma and tags
...
%The relevant modules of the system for this method are the \emph{lexical
...
\item[Breton--French:] Development of the Breton--French pair has been
  ...
  selection.\footnote{The version used in this paper is SVN revision
...
%  and French. It has been developed part-time over a number of years.
  \todomlf{R1 says ``whole section is rather length especially the
    revision numbers and URLs of the specific translations systems
    need not be mentioned in my opinion. The space could rather be
    used to elaborate in the description of the method in Sec
    2.''. FSM: I have moved them to the footnote}
...
%  system. 
  ...
  is much higher % (see table~\ref{table:lang-pair-stats})
...
%  over a period of around four months. The pair
...
\todomlf{4-month mention removed in response to R1}
\end{description}
...
that sentence as\todomlf{R3 says: ``The authors use LER, but F-score measures seem more appropriate to estimate the performance of the disambiguation module. Moreover, the constraint of the disambiguation module to generate only one word when evaluating seems extreme.'' He don't get it.}
\begin{equation}
...
that,\todomlf{R1 says ``this has already been mentioned
  before''. Don't know what to do FSM: I would do nothing, this is why
the sentence says ``Recall ...''} although $T_t(s_i)$ returns a set,
this set will be a singleton, as when the lexical-selection module
returns more than one translation the first one will be
selected.\footnote{In practice this does not happen as each ambiguous
  word has a \emph{default} translation.}\todomlf{R1 complains: ``why
  mention if it doesn't happen in practice? Relevance?'' What should
  we do?}
...
overview of the inputs.\todomlf{Caption of figure extended in response to R3} In the description it is assumed that the
...
widely-used {\sc bleu} metric \citep{papineni02}.\todomlf{BLEU reference advanced, and wrong section link removed} This is not ideal for evaluating the
...
in English sentences with their % a selection of
...
which is found aligned in the reference.
  ...
  %% FMT: INCLUDE AVERAGE AMBIGUITY OVER AMBIGUOUS WORDS
  \begin{table}
  \begin{center}
...
%    change so much from the paper?} 
Table~\ref{table:test-corp} gives
  information about the test corpora. 
%! \todo{FSM: One may wonder which
...
to calculate the feature weights.  \todomlf{R3 says: ``The proposed
  maximum entropy or log-linear model is trained in a surprising
  way. In the early days of maximum entropy models, those were trained
  using the GIS algorithm. However, it has been proved in the
  literature that GIS has clear disadvantages with respect to other
  batch gradient descent algorithms. For instance, it does not allow
  for the use of regularization [...], and it is extremely slow to
  converge, which yields worse models than other methods. The
  smoothing/regularization of the model is also poor since it is
  simply feature pruning instead of regularization.'' FSM: I would say
  something in the discussion (future work).}  After learning the
...
%tables~\ref{table:eval-para-results} and \ref{table:eval-mono-results}.
 ...
 different domain than that of the corpus.\todomlf{R1 says the
   footnote is important and should be incorporated to the text; what
   should we do? FSM: I have moved it to the main text and split the
   paragraph into two pieces.}
...
% \todo{I miss a brief separate discussion of BLEU and LER, which are reported separately!}
...
the TL model online.\todomlf{R2 says: ``An error analysis would also
  help to understand the proposal.'' We don't have time now I think.}
