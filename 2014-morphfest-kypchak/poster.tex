\documentclass[fontscale=0.3,landscape,paperwidth=72in,paperheight=36in]{baposter}  % fontscale=0.285, dvipdfm
...
%\definecolor{headercol}{RGB}{6,130,22}
...
%%			\draw (current page.north west)+(-2em,2em) node[anchor=north west] {\includegraphics[width=0.9\textwidth]{flagkg2}};
  ...
  %headerfont=\Large\textsf, %Sans Serif
			...
			%\hspace{-2em}
			\includegraphics[angle=90,height=6.5em]{apertium5b}%\hspace{-2em}
...
%		}
	...
	\scalebox{0.78}{
		\begin{tabular}{lllll}
			\toprule
				 & \textbf{Kyrgyz} & \textbf{Kazakh} & \textbf{Tatar} & \textbf{Kumyk} \\
				 & /{\qipa qɯɾʁɯz}/ & /{\qipa qɑzɑq}/ & /{\qipa tɒtɑɾ}/ & /{\qipa qumuq}/ \\
...
%		\end{itemize}
	...
	\headerbox{Development effort}{name=devel,column=4,row=0}{
	\scalebox{0.92}{
		\begin{tabular}{lllll}
			\toprule
			 & \textbf{Kyrgyz} & \textbf{Kazakh} & \textbf{Tatar} & \textbf{Kumyk} \\
			...
			80\% cov.\ & Aug.?\ 2011 & Aug.\ 2012 & Aug.\ 2012 & Oct.\ 2013 \\\midrule
			time & 4 months & 19 months & 7 months & 1 week \\
			\bottomrule
		\end{tabular}
	}
		\begin{itemize}
			%\item Kyrgyz and Kazakh transducers begun in 2011
			%\item Kyrgyz transducer reached 80\% coverage summer of 2011
			\item (various periods of intermission, various rewrites)
			\item Kazakh transducer based on Kyrgyz transducer
			\item Kyrgyz transducer currently being rewritten based on insights gained while writing other Turkic transducers
			\item Kumyk transducer based on Kazakh, Tatar transducers:\\
			\tilda{}1 week to reach 80\% coverage, +1 week to reach 90\%
		\end{itemize}
		\vspace*{-2.5em}
		\hspace*{-2em}\includegraphics[width=0.9\textwidth,angle=270]{../kipchak-lrec-2013/graphs/kumyk-yoldash-cov}
		\vspace*{-2em}
	}
...
%		}
		...
		\pex[aboveexskip=0pt,belowexskip=3pt,aboveglftskip=0pt]\label{ex:sent}
			\begingl
				\gla[everygla={\qipa }] Кудай Өзү жаратканынын баарына карап, {} өтө жакшы экенин көрдү. //
				\glb[everyglb={\qipa }] Құдай Өзінің жаратқандарының бәріне қарап, {} өте жақсы екенін көрді. //
				...
				\glft \eng{God looked at everything he had created and saw that it was very good.} (Bible, Genesis 1:31) //
			\endgl
		\xe
...
\begin{tabular}{llll}
\toprule
 \textbf{Kyrgyz} (\textbf{kir}) & \textbf{Kazakh} (\textbf{kaz}) & \textbf{Tatar} (\textbf{tat}) & \textbf{Kumyk} (\textbf{kum}) \\
\midrule
\texttt{Кудай Өзү жаратканынын баарына} & \texttt{Құдай Өзінің жаратқандарының бәріне} & \texttt{Аллаһ Үзе яраткан нәрсәләргә карап,} & \texttt{Аллагь Оьзю яратгъан затлагъа} \\
\texttt{карап, өтө жакшы экенин көрдү.} & \texttt{қарап, өте жақсы екенін көрді.} & \texttt{аларның бик яхшы икәнен күрде.} & \texttt{къарап, олар бек яхшы экенин гёрген.} \\
\midrule
	\texttt{Кудай\tag{n}\tag{nom}} & \texttt{Құдай\tag{n}\tag{nom}} & \texttt{Аллаһ\tag{n}\tag{nom}} & \texttt{Аллагь\tag{n}\tag{nom}} \\
	\texttt{Өз\tag{prn}\tag{ref}\tag{px3sp}\tag{nom}} & \texttt{Өз\tag{prn}\tag{ref}\tag{px3sp}\tag{gen}} & \texttt{Үз\tag{prn}\tag{ref}\tag{px3sp}\tag{nom}} & \texttt{Оьз\tag{prn}\tag{ref}\tag{px3sp}\tag{nom}} \\
	\texttt{жарат\tag{v}\tag{tv}\tag{ger\_past}\tag{px3sp}\tag{gen}} & \texttt{жарат\tag{v}\tag{tv}\tag{ger\_past}\tag{pl}\tag{px3sp}\tag{gen}} & \texttt{ярат\tag{v}\tag{tv}\tag{gpr\_past}} & \texttt{ярат\tag{v}\tag{tv}\tag{gpr\_past}} \\
	\texttt{баары\tag{prn}\tag{qnt}\tag{px3sp}\tag{dat}} & \texttt{бәрі\tag{prn}\tag{qnt}\tag{px3sp}\tag{dat}} & \texttt{нәрсә\tag{n}\tag{pl}\tag{dat}} & \texttt{зат\tag{n}\tag{pl}\tag{dat}} \\
	\texttt{кара\tag{v}\tag{tv}\tag{gna\_perf}} & \texttt{қара\tag{v}\tag{tv}\tag{gna\_perf}} & \texttt{кара\tag{v}\tag{tv}\tag{gna\_perf}} & \texttt{къара\tag{v}\tag{tv}\tag{gna\_perf}} \\ 
	\texttt{,\tag{cm}} & \texttt{,\tag{cm}} & \texttt{,\tag{cm}} & \texttt{,\tag{cm}} \\
	 --- & ---      & \texttt{алар\tag{prn}\tag{pers}\tag{p3}\tag{pl}\tag{gen}} & \texttt{олар\tag{prn}\tag{pers}\tag{p3}\tag{pl}\tag{nom}} \\
	\texttt{өтө\tag{adv}} & \texttt{өте\tag{adv}} & \texttt{бик\tag{adv}} & \texttt{бек\tag{adv}} \\
	\texttt{жакшы\tag{adj}} & \texttt{жақсы\tag{adj}} & \texttt{яхшы\tag{adj}} & \texttt{яхшы\tag{adj}} \\
	\texttt{э\tag{cop}\tag{ger\_past}\tag{px3sp}\tag{acc}} & \texttt{е\tag{cop}\tag{ger\_past}\tag{px3sp}\tag{acc}} & \texttt{и\tag{cop}\tag{ger\_past}\tag{px3sp}\tag{acc}} & \texttt{э\tag{cop}\tag{ger\_past}\tag{px3sp}\tag{acc}} \\
	\texttt{көр\tag{v}\tag{tv}\tag{ifi}\tag{p3}\tag{sg}} & \texttt{көр\tag{v}\tag{tv}\tag{ifi}\tag{p3}\tag{sg}} & \texttt{күр\tag{v}\tag{tv}\tag{past}\tag{p3}\tag{sg}} & \texttt{гёр\tag{v}\tag{tv}\tag{past}\tag{p3}\tag{sg}} \\  
	\texttt{.\tag{sent}} & \texttt{.\tag{sent}} & \texttt{.\tag{sent}} & \texttt{.\tag{sent}} \\
\bottomrule
\end{tabular}
}
...
%  \texttt{{\small <px3sg>}} \> 3rd person poss. (Singular)\\[-0.5ex]
  ...
  \texttt{{\small \tag{sent}}} \hspace{0.2em} \= Sentence\\[-0.5ex]
...
\end{multicols}
		...
		\headerbox{Categorisation}{name=adjectives,column=4,below=devel}{
...
ADV-ABL ;
		...
		\headerbox{Evaluation}{name=coverage,span=1,column=5,row=0}{
			...
			\begin{tabular}{lrrrr}
				\toprule
				\multirow{2}{*}{\textbf{Part of speech}} & \multicolumn{4}{c}{\textbf{Number of stems}} \\ \cmidrule{2-5}
				& \textbf{Kyrgyz} & \textbf{Kazakh} & \textbf{Tatar} & \textbf{Kumyk} \\
				\midrule
				...
				\midrule
				Total: & \hiliteone{13749} & \hiliteone{11224} & \hiliteone{10737} & \hiliteone{4845} \\
				\bottomrule
			\end{tabular}
			\end{center}
			%\vspace{2em}
...
%		}
		\scalebox{0.8}{
			\begin{tabular}{llll}
				...
				\midrule
					\textbf{Kyrgyz} & Wikipedia & azattyk.org & Bible \\
					% 2013-05-30, 2010, 
					\textbf{Kazakh} & Wikipedia & azattyq.org & Quran + Bible \\
...
%				Precision is the probability that an analysis provided by the transducer is accurate.
%				Recall is the probability that some correct analysis is among those provided.
...
\begin{center}
		\scalebox{0.94}{
			\begin{tabular}{llrrr}
				\toprule
				 & \textbf{Corpus} & \textbf{Tokens} & \textbf{Coverage} (\%) & \textbf{Amb.} \\
				\midrule
...
%			\end{center}
		...
		\headerbox{Orthography-phonology mapping issues}{name=twol,column=3}{ %,below=exout}{
			\htwo{Ambiguous characters}
			\begin{itemize}
				\item Have front- and back-vowel readings in native words
			\end{itemize}

		%\scalebox{0.92}{
			\begin{tabular}{llll}
				\toprule
					& \textbf{letters} & \textbf{values} & \textbf{examples} \\
				\midrule
					\multirow{2}{*}{\texttt{\textbf{kaz}}} & \multirow{2}{*}{{\qipa и, у, ю}} & {\qipa /əj, əw, jəw/} & {\qipa қ\hiliteone{ию}д\hilitetwo{а}} \eng{chopping down} \\
						&	& {\qipa /ɘj, ɘw, jɘw/} &  {\qipa к\hiliteone{ию}д\hilitetwo{е}} \eng{getting dressed} \\
				\midrule
		...
		\vspace{0.5em}
			...
			\htwo{Loanwords}
				\begin{itemize}
					\item Letters that represent front vowels in native words may represent ``back'' vowels in Russian words
				\end{itemize}
			\scalebox{0.92}{
				\begin{tabular}{lll}
					\toprule
						 & \textbf{native word example} & \textbf{Russian word example} \\
					\midrule
						\texttt{\textbf{kaz}} & {\qipa \hiliteone{е}лд\hilitetwo{і}ң} \eng{country's} & {\qipa Назарба\hiliteone{е}вт\hilitetwo{ы}ң} \eng{Nazarbayev's} \\
						\texttt{\textbf{tat}} & {\qipa гал\hiliteone{и}мн\hilitetwo{ә}р} \eng{scientists} & {\qipa арт\hiliteone{и}стл\hilitetwo{а}р} \eng{artists} \\
						\texttt{\textbf{kum}} & {\qipa с\hiliteone{ё}зл\hilitetwo{е}р} \eng{words} & {\qipa самол\hiliteone{ё}тл\hilitetwo{а}р} \eng{airplanes} \\
					\bottomrule
				\end{tabular}
			}
			\begin{itemize}
				\item solution:\ separate continuation lexicon (messy rules)
			\end{itemize}
				\codeex{
					\begin{spacing}{0.9}
					\texttt{LEXICON N1-RUS\\
					:\%\{☭\%\} N1 ;\\
					\vspace*{-0.5em}\\
					LEXICON Nouns\\
					артист:артист N1-RUS ; ! "artist"\\
					галим:галим N1 ; ! "scientist"}\end{spacing}
					\vspace{0em}
				}\\

			\vspace{-0.75ex}
			\htwo{Acronyms and numerals}
				\begin{itemize}
					\item \texttt{twol} rules handle phonology for spelt-out words\\
					{\qipa от\hiliteone{ыз}\hilitetwo{да}н} \eng{from thirty}, {\qipa б\hiliteone{ес}\hilitetwo{те}н} \eng{from five}
					\item no phonological triggers available in numerals\vspace{-0.2em}\\
					 (incorrect phonological triggers in acronyms)\\
					{\qipa 30-\hilitetwo{да}н} \eng{from 30}, {\qipa 5-\hilitetwo{те}н} \eng{from 5}
					\item solution:\ phonology-triggering characters
					\item simplified:\ e.g., \archiphon{с} for all voiceless ostruents
				\end{itemize}
				\codeex{
					\begin{spacing}{0.9}
					\texttt{4:4\%\{э\%\}\%\{с\%\}           NUM-DIGIT ; ! "төрт"\\
					5:5\%\{э\%\}\%\{с\%\}           NUM-DIGIT ; ! "бес"\\
					3\%0:3\%0\%\{а\%\}\%\{з\%\}       NUM-DIGIT ; ! "отыз"}\end{spacing}
					\vspace{0em}
				}\\
			...
			\htwo{й + vowel letters}
				\begin{itemize}
					%\item Stem-final й combines with following а, о, у to form я, ё, ю
					\item{} [ а о у ] become [ я ё ю ] after й and й deletes
					\item й incorporated into the context of many rules
					\item additional rules to change the characters and delete original й
				\end{itemize}
%	\vspace{-1ex}
%
				\codeex{
...
%				\end{itemize}
			...
			\htwo{A resulting messy \texttt{twol} rule}
				\codeex{
					\begin{spacing}{0.4}
					RdYotVow = ё ю Ё Ю ; \\
					AbstractVow = \%\{а\%\} \%\{э\%\} \%\{ү\%\} \%\{о\%\} ; \\
...
\%\{A\%\}:е <=> \hfill{} [ [ :FrontVow | [ :Vow :ь ] ] :Cns :Cns* ]/:0 \_ ; \\
\hspace*{0.1em}\hfill{}                        [ [ :RdVow :ь ] :Cns :Cns* ]/:0 \_ ; \\
\hspace*{0.1em}\hfill{}     [ [ [ \textbackslash{}[ .\#.\ | :Vow ] ] :RdYotVow ] :Cns :Cns* ]/:0 \_ ; \\
\hspace*{0.1em}\hfill{}      [ :RdYotVow й:0 :RdYotVow :Cns :Cns* ]/[ :0 - й:0 ] \_ ; \\ % ! кююню \\
\hspace*{0.1em}\hfill{} [[\%\{э\%\}:0|\%\{ү\%\}:0] :Cns* ]/[[ :0 - AbstractVow: ] | \%-: ]* \_ ; \\
\hspace*{4em}        except \\
\hspace*{0.1em}\hfill{}       [ :RdYotVow :Cns* \%\{☭\%\}:0 :Cns* ]/[ :0 - \%\{☭\%\}:0 ] \_ ; \\ % ! -ёрлар in Russian words \\
\hspace*{0.1em}\hfill{}                          [ :Cns :р \%\{☭\%\}: \%>: :Cns* ]/:0 \_ ; \\% ! центр and тигр take back vowels \\
\hspace*{0.1em}\hfill{}         [ [ :Vow - :RdYotVow ] :RdYotVow :Cns :Cns* ]/:0 \_ ; \\% ! for some reason the rule above doesn't exclude this condition  e.g., боюнгъа \\
\hspace*{0.1em}\hfill{}                          [ :Vow ]/[ [ :0 - й:0 ] | \%>: ] \_ ;\end{spacing}
			\vspace{1em}
		...
		\headerbox{Ongoing and future work}{name=nedostatki,column=5,below=coverage}{
			\begin{itemize}
...
%					{\qipa же- \eng{eat} → жеш / *жөө \eng{eating}}
		...
		\headerbox{Further information}{name=getting,column=0,below=morphtrans}{
			\begin{itemize}
				\item Part of \textbf{Apertium Turkic} project:\\
				\url{https://wiki.apertium.org/wiki/Apertium\_Turkic}
				...
				%info at {\small \url{https://wiki.apertium.org/wiki/apertium-kir}}
...
%		}
		...
		\headerbox{Morphophonology}{name=morphophon,column=1,below=exout}{
%			\htwo{Vowel harmony}
%			\begin{itemize}
...
%				}
		...
		\headerbox{Other uses}{name=uses,column=1,below=morphophon}{
			\begin{itemize}
				\item HFST transducers are trivially converted to \textbf{spell checkers}
				\item Segmenter, e.g.\ {\qipa көргөзгөндөрдөнсүңбү} :\\
				 {\small \texttt{көр\hilitegrey{>}\{G\}\{A\}з\hilitegrey{>}\{G\}\{A\}н\hilitegrey{>}\{L\}\{A\}р\hilitegrey{>}\{D\}\{A\}н\hilitegrey{>}с\{I\}ң\hilitegrey{>}\{B\}\{I\}}}
				 
				 %көр\hilitegrey{>}\{I\}ш\hilitegrey{>}\{G\}\{A\}н\hilitegrey{>}\{L\}\{A\}р\hilitegrey{>}\{I\}м\hilitegrey{>}\{D\}\{A\}н\hilitegrey{>}с\{I\}ң\hilitegrey{>}\{B\}\{I\}}}
			\end{itemize}
		}

		\headerbox{Morphotactics}{name=lexc,column=2,below=exout}{
			\htwo{Morphological \& orthographical words}

			\begin{itemize}
				\item{} {\qipa өнүктүрөбүзбү ?} \eng{will we develop [it]?}\\
				\texttt{өнүк\tag{v}\tag{tv}\tag{caus}\tag{aor}\tag{p1}\tag{pl}+бы\tag{qst}}
				\item{} {\qipa келатсаң} \eng{if you come}\\
				\texttt{кел\tag{v}\tag{iv}\tag{prc\_impf}+жат\tag{vaux}\tag{gna\_cnd}\tag{p2}\tag{sg}}
			\end{itemize}
			...
			\begin{itemize}
				\item Some combinations of possessive + case morphemes are unpredicted (i.e., not formed simply by concatenation and application of phonology):
			\end{itemize}
...
{\qipa		
	\begin{tabular}{llllll}
		...
		\midrule
		nominative & — & -(I)м & -(I)ң & -(с)I  \\
		accusative & -NI & -(I)мдI & -(I)ңдI & -\textbf{(с)Iн}  \\
		genitive & -NIн & -(I)мдIн & -(I)ңдIн & -(с)IнIн  \\
		locative & -DA & -(I)мдA & -(I)ңдA & -\textbf{(с)IндA}  \\
		ablative & -DAн & -(I)мдAн, & -(I)ндAн, & -\textbf{(с)IнAн}  \\
		~   &     & -\textbf{(I)мAн} & -\textbf{(I)ңAн} \\
		dative & -GA & -\textbf{(I)мA} & -\textbf{(I)ңA} & -\textbf{(с)IнA} \\
		\bottomrule
	\end{tabular}\\
}}
	{\footnotesize A,I,N,D,G have various allophones; (I) null after vowels; (с) null after cons.}\vspace{-1em}\end{center}

%				\item Trade-off:
				\begin{itemize}
					\item underlying \tag{px3sp} form used:\ \texttt{\archiphon{s}\archiphon{I}\archiphon{n}}
					\item \texttt{\archiphon{s}} and \texttt{\archiphon{n}} default to с and н; rules map to null by context
					\item morphophonology more complicated, morphotactics simpler
%				\end{itemize}
			\end{itemize}
			...
			\vspace{0.25em}
			\begin{itemize}
				\item a N-N compund type:\ N2 has \tag{px3} and related morphology\\
				e.g., \texttt{аба ырайы\tag{n}\tag{loc}:\ \hiliteone{аба ырайында}, *\hilitetwo{аба ырайыда}}
			\end{itemize}
			\vspace{0.8ex}
			\codeex{
			\begin{spacing}{0.9}
			...
			\texttt{LEXICON Nouns\\
			\vspace{-0.5ex}аба\%\ ырайы:аба\%\ ырай\ N-INFL-3PX-COMPOUND ;\\\vspace{-0.5ex}\hspace{0.1pt}\hfill{}!\ "weather"\\
			\vspace{-0.5ex}чакыруу\%\ кагазы:чакыруу\%\ кагаз\ N-INFL-3PX-COMPOUND\\\vspace{-0.5ex}\hspace{0.1pt}\hfill{};\ !\ "invitation"}}\end{spacing}
			\vspace{0.5ex}}
