...
%		}
		...
		\headerbox{Development cycle}{name=development,below=morphtrans}{
			...
			\begin{tikzpicture}[node distance = 2cm, auto]
				% Place nodes
				\node [block] (init) {analyse corpus};
				\node [cloud, left of=init] (addstem) {add stem};
				\node [cloud, right of=init] (fixphon) {fix phonology};
				\node [block, below of=init] (freq) {examine frequency list};
				%\node [block, below of=identify] (evaluate) {evaluate candidate models};
				%\node [block, left of=evaluate, node distance=3cm] (update) {update model};
				\node [decision, below of=freq] (decide) {cause for unanalysed forms?};
				%\node [block, below of=decide, node distance=3cm] (stop) {stop};
				% Draw edges
				\path [line] (init) -- (freq);
				\path [line] (freq) -- (decide);
				\path [line] (decide) -| node [near start] {missing stem} (addstem);
				\path [line] (decide.east) -| node [near start] {bad phonology} (fixphon);
				\path [line,dashed] (addstem) -- (init);
				\path [line,dashed] (fixphon) -- (init);
			\end{tikzpicture}
		}
