		...
		\headerbox{Example:\ Tuvan velar elision}{name=tuvanvelars,column=1,span=2}{
			\begin{multicols}{2}
				\htwo{Previous descriptions}
					...
					\begin{itemize}
						\item \textcite[22-23]{andersonharrison99} %Anderson and Harrison (1999, pp.\ 9, 22-23)
					\end{itemize}
...
%\-~~~~~~~~:Vow \_ [ \%\{☭\%\}:\ :Vow ]/:0* ;
	...
	\headerbox{Example: Categorisation}{name=adjectives,column=1,below=tuvanvelars}{
...
%	}
		...
		\headerbox{Example: Tuvan vowel harmony}{name=tuvanvowelharmony,column=3}{ %{name=otherexamples,column=1,span=2,below=tuvanvelars}{ % after consonant clusters in loanwords
			%\begin{multicols}{2}
...
%		}
		...
		\headerbox{References}{name=references,column=3,below=tuvanvowelharmony}{
			\begin{spacing}{0.7}
		...
		\headerbox{Development cycle}{name=development,below=morphtrans}{
			...
			%\scalebox{0.65}{
			\begin{tikzpicture}[xscale=1.92,yscale=2.25]%, auto]	

				\node[cloud] (assumptions) at (1,2.75) {assumed generalisations};
				\node[block] (analyse) at (1,2) {analyse corpus};
				...
				\path[line] (phonbetter.east) -- node [below=1ex, near start, scale=\flowchartscale] {yes} (4.5,0) |- (newgeneralisation.east) ;
				\path[line] (morphbetter.east) -- node [below=1ex, near start, scale=\flowchartscale] {yes} (4.5,1) |- (newgeneralisation.east) ;
				...
				\path[line] (newgeneralisation) -- (analyse);
