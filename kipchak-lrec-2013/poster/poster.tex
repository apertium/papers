	...
	\scalebox{0.85}{
		\begin{tabular}{llll}
			\toprule
				...
				classific & S Kypchak & N Kypchak & W Kypchak \\
			\midrule
...
%		\end{itemize}
	...
	\headerbox{Morphological transducers}{name=morphtrans,below=kypchaklgs}{
		...
		\begin{itemize}
			...
			\item Kyrgyz (\hiliteone{Tyers et al., 2012}) \hfill{} \hiliteone{GPL (=free and open)}!
		\end{itemize}
...
%		}
		...
		%\headerbox{Morphophonology}{name=morphophon,below=morphtrans}{
			\begin{itemize}
				\item Other Turkic transducers: zero-derivation (overgenerates)
				\item Our approach: categorization
			\end{itemize}

		\scalebox{0.65}{
			\begin{tabular}{lllll}
				\toprule
				\textbf{Type} & \textbf{Example} & {\footnotesize \texttt{\tag{adj}(\tag{comp})}} & {\footnotesize \texttt{\tag{adj}\tag{subst}(\tag{comp})}} & {\footnotesize \texttt{\tag{adj}\tag{advl}(\tag{comp})}} \\
				\midrule
				A1 & \emph{`good'} & {\qipa яхшы (яшхырак)} & {\qipa яхшы (яхшырак)} & {\qipa яхшы (яхшырак)} \\
				A2 & \emph{`old'} & {\qipa иске (искерәк)} & {\qipa иске (искерәк)} &  \\
				A3 & \emph{`dead'} & {\qipa үле} & {\qipa үле } &  \\
				A4 & \emph{`basic'} & {\qipa төп} &  &  \\
				\bottomrule
			\end{tabular}
		}
