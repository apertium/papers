...
%		}
		...
		\scalebox{0.63}{
			\begin{tabular}{lllll}
				...
				\midrule
				A1 & \eng{good} & {\qipa яхшы (яхшырак)} & {\qipa яхшы (яхшырак)} & {\qipa яхшы (яхшырак)} \\
				A2 & \eng{old} & {\qipa иске (искерәк)} & {\qipa иске (искерәк)} & {\qipa — (—)} \\
				A3 & \eng{dead} & {\qipa үле (—)} & {\qipa үле (—)} & {\qipa — (—)} \\
				A4 & \eng{basic} & {\qipa төп (—)} & {\qipa — (—)} & {\qipa — (—)} \\
				\bottomrule
			\end{tabular}
		}
	}
...
%			\end{center}
		...
		\scalebox{0.85}{
			\begin{tabular}{llll}
				...
				\bottomrule
					\multirow{2}{*}{\textbf{Kazakh}} & \multirow{2}{*}{{\qipa и, у, ю}} & {\qipa /əj, əw, jəw/} & {\qipa қ\hiliteone{ию}д\hilitetwo{а}} \eng{in the process of chopping down} \\
						&	& {\qipa /ɘj, ɘw, jɘw/} &  {\qipa к\hiliteone{ию}д\hilitetwo{е}} \eng{in the process of getting dressed} \\
					\midrule

					\multirow{3}{*}{\textbf{Tatar}} & \multirow{3}{*}{{\qipa е}} & {\qipa э / C \_} & {\qipa дәр\hiliteone{е}сл\hilitetwo{ә}р} \eng{lessons} \\
						&	& {\qipa /j/+ы} & {\qipa \hiliteone{е}лл\hilitetwo{а}р} \eng{years} \\
						&	& {\qipa /j/+э} & {\qipa ег\hiliteone{е}тл\hilitetwo{ә}р} \eng{boys} \\
					\midrule

					\multirow{3}{*}{\textbf{Kumyk}} & \multirow{3}{*}{{\qipa ё, ю}} & {\qipa /ø, y/ / C \_} & {\qipa г\hiliteone{ё}зл\hilitetwo{е}р} \eng{eyes}, {\qipa г\hiliteone{ю}нл\hilitetwo{е}р} \eng{days}\\
						&	& {\qipa /jø, jy/} & {\qipa \hiliteone{ю}рекл\hilitetwo{е}р} \eng{hearts}, {\qipa \hiliteone{ё}нкюл\hilitetwo{е}р} \eng{darlings} \\
						&	& {\qipa /jo, ju/} & {\qipa \hiliteone{ю}лдузл\hilitetwo{а}р} \eng{stars}, {\qipa \hiliteone{ё}лл\hilitetwo{а}р} \eng{roads} \\
				\bottomrule
			\end{tabular}
		}
			Tatar - е (non-initial orthographic variant of э, e.g. дәресләр)
				- initial position /j/+э (егетләр) and /j/+ы (еллар)
			Kumyk - ё ю (front round vowels between consonants гёзлер, гюнлер)
				- word-initially /j/ followed by front (юреклер, ёнкюлер) or back vowel (юлдузлар, ёллар)
			Kazakh - и у ю (front/back vowels, along with glides)
				- қиюда/киюде
			\htwo{Loanwords}
				Kazakh - елдің / Назарбаевтың
				Tatar - галимнәр, артистлар
				Kumyk - сёзлер, самолётлар
				- separate continuation lexicon
				- result: super messy twol rules
