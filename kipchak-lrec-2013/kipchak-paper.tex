...
\caption{An example of the output of each of the morphological transducers for the same 
  ...
  very good.'', Genesis 1:31). The sentence may be very roughly glossed as ``God own created things to looking,
  their good being saw''. The output has been 
  abbreviated to only show the appropriate tag in context, 
  the actual output would be ambiguous.}
...
\begin{itemize}
 \item \textbf{Ambiguous and `irregular' harmony}: In Tatar, certain characters in the alphabet which represent vowels can be 
   ambiguous between a vowel and a `yoticised' vowel, such as е, which is front or back. For 
   example: дәрес `lesson', дәресләр `lessons'; ел `year', еллар `years'. 
 \item \textbf{Loan words}: In Kumyk, the character ё in Russian words is pronounced as in Russian where in native words 
