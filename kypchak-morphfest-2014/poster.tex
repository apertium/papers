...
%		\end{itemize}
	...
	\headerbox{Morphological transducers}{name=morphtrans,below=kypchaklgs}{
		...
		\begin{itemize}
			...
			\item compiled separately; compose-intersected to single transducer\\
			алдым \hilitetwo{↔} \texttt{ал\hilitegrey{>}\archiphon{D}\archiphon{I}\hilitegrey{>}м} \hilitetwo{↔} \texttt{ал\tag{v}\tag{tv}\tag{ifi}\tag{p1}\tag{sg}}
		\end{itemize}
...
\end{multicols}
		...
		%\headerbox{Morphophonology}{name=morphophon,below=morphtrans}{
			\begin{itemize}
				\item Example:\ morphologically distinct adjective classes (not documented elsewhere)
				\item Other Turkic transducers:\ \texttt{0}-derivation (\hilitetwo{overgenerates})
				\item Our approach:\ categorisation (generates/analyses only correct forms)
			\end{itemize}
		...
		\vspace{0.5em}
			...
			\htwo{Acronyms and numerals}
				\begin{itemize}
					...
					\item no phonological triggers available in numerals
					\item (incorrect phonological triggers in acronyms)\\
...
%				\item Trade-off:
				\begin{itemize}
					...
					\item phonological rules delete \texttt{\archiphon{s}}, \texttt{\archiphon{n}} by context
