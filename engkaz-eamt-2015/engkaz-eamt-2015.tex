...
\item[Post-generator:] Performs some minor orthographical operations in the target language. This module is not used in the English--Kazakh system.\todo{Or is it?}
...
mektep+ter, etc. \todo{Aida, please typeset this example in Cyrillic; braces are not used here, but are used later; homogeneize notation in paper.}
...
lexical-selection rules depending on context. This dictionary currently contains 13,130 stems \citep{sundetova13a}.\todo{later you say 13,135}
...
place or \todo{or what?} \emph{beautiful mountains} \(\to\) \emph{әдемі таулар}.  Table~\ref{table:lexsel} shows some 
lexical-selection rules:\todo{but this rule is not in the table!!!}
\begin{table*}
 \centering
...
\\ 
 ...
 \end{tabular}
  ...
  \label{table:lexsel}
\end{table*}
\todo{I do not understand the \emph{I see anything} example. Does it mean \emph{anything} as \emph{whatever}? I do not know how to put the \(w\) in this example}@
...
This section describes the current structural transfer in \texttt{apertium-eng-kaz} (revision ..., ).\todo{Complete! 58998? Move to results?} English--Kazakh 
...
\paragraph{Postchunk:}\todo{Nothing is said about postchunk --- this talks about the additional cleaning stage}
...
contains 13,135 entries, \todo{13130 before!} There are 168 ``chunker'' rules and 148 interchunk rules. 
...
As can be seen,\todo{where!} the coverage of dictionary is good, average score is 94\%. Our system outperforms the other 
...
between the Turkic or other language pairs. Related work is currently ongoing with Russian--Kazakh.\todo{Say something about Kazakh--English.}
...
MLF thanks the Kazakh state program for the attraction of foreign scholars and Prof. Ualsher Tukeyev for supporting his visit to the Kazakh National University, where part of this work was carried out. We also thank Prof. Tukeyev for his valuable input.\todo{Some references didn't come out right.}
