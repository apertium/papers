...
\newcommand{\tyv}[1]{\textit{#1}}
\newcommand{\tgloss}[1]{\tyv{#1}}
...
\renewcommand*{\bibfont}{\fontsize{5.735pt}{5.735pt}\selectfont}
...
\begin{document}
	...
	\headerbox{Tuvan}{name=tyvan,column=0,row=0}{
		...
		\begin{itemize}
			...
			\item Spoken in Tuva Republic and nearby
	...
	\headerbox{Morphological Transducers}{name=morphtrans,below=tyvan}{
		...
		\begin{itemize}
			\item Computational models of a language's morphology
			\item Efficient in speed \& size
		...
		\begin{itemize}
			...
			\item \textbf{morphophonology} implemented in \texttt{twol}
		...
		%\end{itemize}
		\begin{itemize}
			\item mostly hand-written; some parts automatable
		\end{itemize}
...
\codeex{\hilitegrey{"\{I\} harmony"\\
	...
	\-~~~~~~~~~~~~[ :BackVow :Cns* :Cns :л ь:0 ]/:[ :0 - ь: ]* \_ ;}
		...
		\centering
		\begin{tikzpicture}[xscale=1.45,yscale=2.19]%, auto]
...
%			\end{tikzpicture}%}
	...
	%	\end{tabular}
		...
		\vspace{-1em}
		\newline
			\tikzoverlay[text width=0.9\textwidth] at (0.55in, -2.25in) {
...
%               1         2         3          4              5          6          7        8
                %Мен \& тыва \& хүреш \& -биле \& сургуул \& кылбастай \& бээримге \& ачам \& хомудаан \& . \\
                Бис \& кожууннуң \& соңгукчулары \& -биле \& ажылды \& чорутпастай \& бээривиске, \& көргүзүглер \& баксыраан \& . \\
%                \gmk{prn} \& \gmk{n.attr} \& \gmk{n.nom} \& \gmk{post} \& \gmk{n} \& \gmk{v.tv.prc} \& \gmk{vaux} \& \gmk{n.px1sg.nom}  \& \gmk{v.iv.past} \&  \\
                \gmk{prn} \& \gmk{n.gen} \& \gmk{n.pl.px3sp} \& \gmk{post} \& \gmk{n.acc} \& \gmk{v.tv.prc} \& \gmk{vaux} \& \gmk{n.pl.nom}  \& \gmk{v.iv.past} \&  \\
                \tgloss{We} \& \tgloss{district} \& \tgloss{constituents} \& \tgloss{with} \& \tgloss{business} \& \tgloss{having stopped conducting} \& \tgloss{} \& \tgloss{figures} \& \tgloss{worsened} \& \\
                \end{deptext}
                \depedge{6}{1}{\udlabel{nsubj}}
                \depedge{3}{2}{\udlabel{nmod:poss}}
                \depedge{3}{4}{\udlabel{case}}
                \depedge{6}{3}{\udlabel{nmod}}
                \depedge{6}{5}{\udlabel{dobj}}
                \depedge{6}{7}{\udlabel{aux}}
                \depedge{9}{8}{\udlabel{nsubj}}
                \depedge{9}{6}{\udlabel{advcl}}
                \depedge{9}{10}{\udlabel{sent}}
                \deproot[edge unit distance=3ex]{9}{\udlabel{root}}
        \end{dependency}}\\
		\setlength\fboxsep{0.5em}
		\setlength\fboxrule{0pt}
		\fbox{\includegraphics[width=0.63\textwidth]{img/korp-2}}
		%\begin{itemize}
		%\end{itemize}
		\vspace{1.8625in}
	...
	\headerbox{References}{name=references,below=results,column=2}{
		...
		\headerbox{Further information}{name=getting,column=0,below=development,span=3}{
			...
			\begin{itemize}
				\item Part of \textbf{Apertium Turkic} project:\ \url{https://wiki.apertium.org/wiki/Apertium\_Turkic}
