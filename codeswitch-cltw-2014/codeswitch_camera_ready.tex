...
\begin{algorithmic}[1]
%\Statex{}%Procedure{}{}
\Statex $\textit{buffer} = [\ ] \text{\ \ \  \#undecided expanding window of chunk}$
\Statex $\textit{chunks} = [\ ] \text{\ \ \  \#decided labelled segment}$
\Statex $\textit{buffer\_language} \gets \textit{lang\_predict}\text{(sentence[0])} \text{\ \ \ \#Language of first word}$
\Statex $\textit{flag} \gets \text{0}$
...
\If {$ \textit{lang\_predict(word)} \text{==} \textit{buffer\_language} $}
        \If {$ \textit{flag} \text{=\ =\ 1} $}
        \Statex $\textit{buffer} += [\textit{word\_d,word}] $
        \Statex $\textit{flag} \gets \text{0}$
        \Else
        \Statex $\textit{buffer} += [\textit{word}] $
        \EndIf
        \EndIf
\If {$ \textit{lang\_prediction(word)} \text{!=} \textit{buffer\_langugae} $}
        \If {$ \textit{flag} \text{=\ =\ 0} $}
        \Statex $\textit{flag} \gets \text{1}$
        \Statex $\textit{word\_d} \gets \textit{word} $
        \Statex $\text{continue}$

        \Else
        \Statex $\textit{chunk} \text{+= \ }  \textit{[(buffer,buffer\_language)]} $
        %\State $\textit{buffer} \gets [] $
        \Statex $\textit{buffer} = [\textit{word\_d,word}] $
        \Statex $\textit{buffer\_language} = \gets \textit{lang\_predict}\text{(word)}$% \text{\#Language of new expanding chunk}$
        \Statex $\textit{flag} \gets \text{0}$
        \EndIf
...
\If {$\textit{length(buffer)} \text{!=0\ } $}
        \Statex $\textit{chunk} \text{+= \ }  \text{[\textit{[(buffer,buffer\_language)]}} $
