...
Restricting the predicted languages to be amongst the two in the dataset was a decision made to identify the language pairs in a better fashion, both by our techniques and by the competitive tools too. Then we used the character bigram and then character trigram probabilities to predict the detected language for each token. Rules in section~\ref{sec:method} were followed for the text processing and chunking part of the method.
%which included that the span of a segment or a code-switched phrase is more than a word, the social media non-language entities such as the hashtags, usernames and urls do not make a difference. 
...
\hline
\texttt{langid-2character} & 50.35 & 42.28 & 45.11  \\
\hline
