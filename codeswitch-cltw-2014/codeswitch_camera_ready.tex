...
\STATE \textrm{buffer} = [\ ] /*Undecided expanding window of chunk*/
\STATE \textrm{chunks} = [\ ] /*Decided labelled segment*/
\STATE \( \textrm{buffer\_language} \gets \textsc{LangPredict}(s[0])\) /* Language of first word */
\STATE \( \textrm{flag} \gets 0 \)
\FORALL {\(w \in s\)}
  \IF {\( \textsc{LangPredict}(w) = \textrm{buffer\_language}\)}
    \STATE \( \textrm{buffer} += [w] \) 
    \STATE \( \textrm{flag} \gets 0 \)
  \ELSE 
    \STATE \( \textrm{buffer} += [w] \)
  \ENDIF
  ...
  \IF {\( \textsc{LangPredict}(w) \not{=} \textrm{buffer\_language}\)}
    \IF {\( \textrm{flag} = 0 \)}
      \STATE \( \textrm{flag} \gets 1 \)
    \ELSE
      \STATE \( \textrm{chunks} += [(\textrm{buffer},\textrm{buffer\_language})] \)
      \STATE \( \textrm{buffer} = [] \)
      \STATE \( \textrm{buffer\_language} \gets \textsc{LangPredict}(w) \) /* Language of new expanding chunk */
      \STATE \( \textrm{flag} \gets 0 \)
    \ENDIF
  \ENDIF
\ENDFOR
\IF {\( \textsc{len}(\textrm{buffer}) > 0 \)}
  \STATE \textrm{chunks} += [(\textrm{buffer},\textrm{buffer\_language})]
\ENDIF
\end{algorithmic}
\end{algorithm}
