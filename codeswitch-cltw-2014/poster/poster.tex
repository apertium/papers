  ...
  %headerfont=\Large\textsf, %Sans Serif
	...
	\headerbox{Introduction}{name=kypchaklgs,column=0,row=0}{
		...
		\begin{itemize}
			...
			\item We are focussing on cases where - A single sentence may have different \textbf{code switching} points 
		\end{itemize}
		\vspace{0.5cm}
	}
...
of \{`\_ s', `s l', `l \'{a}', `\'{a} i', `i n', `n t', `t e', `e !', `! \_'\}.
		...
		\newline
		\begin{itemize}
			\item	Using \emph{langid.py} labeled all the lines in a particular dataset according to the majority classification 
		\newline
		\end{itemize}

		\htwo{\emph{Langid} character trigram prediction}
		\newline
		\begin{itemize}
			\item	Trigram probabilities from \emph{langid} were taken into account. 	
		\newline
		\vspace{-0.3cm}
		...
		\headerbox{Examples of code-switching segmentation}{name=examplescode,column=1,row=0,span=2}{
		\begin{center}
		...
		\headerbox{Example}{name=examples,column=1,below=examplescode,span=2}{
		%	\begin{table}[h]
			\begin{center}
			\begin{tabular}{ll}
		...
		\headerbox{Chunking algorithm}{name=lexc,column=1,below=examples,span=2}{
...
%		\end{figure}
		...
		\headerbox{Results}{name=results,column=1,span=2,below=lexc}{
		...
		\caption{Precision, $p$ and recall, $r$ for the systems by language.}
			...
			\hline
			\texttt{\emph{langid}-3character} & 57.24 & 45.92 & 43.16 \\
			\hline
		...
		\begin{itemize}
		\newline	
			\item	We followed the footsteps of \textbf{CoNLL 2000} shared task on language independent named entity recognition. 
		\newline	
			\item	Divide the text into non-overlapping segments.
		\newline	
			\item	\textbf{Precision} - Percentage of correctly detected phrases.
		\newline	
			\item	\textbf{Recall} - Number of phrases in the data that were found by the chunker.
		\newline
			\item	\textbf{Accuracy} - Total percentage of words correctly classified.	
		\end{itemize}
		\vspace{0.2cm}
		}
