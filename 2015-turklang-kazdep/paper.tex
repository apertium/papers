...
As seen in the table, the morphologically unmarked nominative case (e.g., үй \gloss{the/a house} \gmk{nom}) has a wide variety of uses, including indefinite object and genitival.  Definite objects are marked with the accusative case (e.g., үйді \gloss{the house} \gmk{acc}), and definite genitivals are marked with the genitive case (e.g., үйдің \gloss{of the house} \gmk{gen}).  The Kazakh transducer marks all bare nominals both as \tag{nom} and \tag{attr}, but the various functions may be disambiguated syntactically.  Genitival nominals, whether definite or not, must (NO EXCEPTIONS) have a corresponding nominal with possessive morphology that agrees in person and number (e.g., үйдің есіг\textbf{і} \gloss{the/a door to the house}; үй тапсырма\textbf{сы} \gloss{homework}), and subject nominals must have a corresponding predicate containing e.g., a verb or a copula that agrees in person and number with the nominative nominal (e.g., үй көшіріле\textbf{ді} \gloss{the house gets moved}).  When a nominative nominal depends on another nominal that does not have possessive case, the first one must be considered attributive (e.g., үй киім \gloss{house clothes}).

The attributive use of nominals is similar to the use of adjectives, and could even be thought of as a ``zero-derivation'' of nouns into adjectives.  Interestingly, many adjectives can also be used substantively.  Table~\ref{tab:zeroderiv} shows the various functions of nominals, adjectives, and adverbs.
...
\begin{small}
        \begin{tabular}{ll|ll|ll}
                ...
                \midrule
\texttt{acl} & clausal modifier of noun & \texttt{csubjpass} & clausal passive subject & texttt{neg} & negation modifier\\
\texttt{advcl} & adverbial clause modifier & \texttt{dep} & unspecified dependency & \texttt{nmod} & nominal modifier\\
\texttt{advmod} & adverbial modifier & \texttt{det} & determiner & \texttt{nsubj} & nominal subject\\
\texttt{amod} & adjectival modifier & \texttt{discourse} & discourse element & \texttt{nsubjpass} & passive nominal subject\\
\texttt{appos} & appositional modifier & \texttt{dislocated} & dislocated elements & \texttt{nummod} & numeric modifier\\
\texttt{aux} & auxiliary & \texttt{dobj} & direct object & \texttt{parataxis} & parataxis\\
\texttt{\sout{auxpass}} & passive auxiliary & \texttt{expl} & expletive & \texttt{punct} & punctuation\\
\texttt{case} & case marking & \texttt{foreign} & foreign words & \texttt{remnant} & remnant in ellipsis\\
\texttt{cc} & coordinating conjunction & \texttt{goeswith} & goes with & \texttt{reparandum} & overridden disfluency\\
...
\texttt{cop} & copula & \texttt{mwe} & multi-word expression & \\
\texttt{csubj} & clausal subject & \texttt{name} & name & \\
