...
As seen in the table, the morphologically unmarked nominative case (e.g., үй \gloss{the/a house} \gmk{nom}) has a wide variety of uses, including indefinite object and genitival.  Definite objects are marked with the accusative case (e.g., үйді \gloss{the house} \gmk{acc}), and definite genitivals are marked with the genitive case (e.g., үйдің \gloss{of the house} \gmk{gen}).  While the Kazakh transducer marks these all as \tag{nom}, their use may be disambiguated.  Genitival nominals, whether definite or not, must (NO EXCEPTIONS) have a corresponding nominal with possessive morphology that agrees in person and number (e.g., үйдің есіг\textbf{і} \gloss{the/a door to the house}), and subject nominals must have a corresponding predicate containing e.g., a verb or a copula that agrees in person and number with the nominative nominal (e.g., үй көшіріле\textbf{ді} \gloss{the house gets moved}).
