...
wordlists containing `known' and `unknown' wordforms.\footnote{\newcite{hall_sproat_russian_2013} randomly selected their training
...
so a perfect stress placement tool has a 50/50 chance of getting them right.
This means that a perfect stress placement tool could achieve 99.48\% on their
task.} In our corpus of running text, at least 7.5\% of the tokens are stress-
ambiguous. Stress ambiguity is
therefore seven times more prevalent in our corpus of running text than it is in 
our wordform dictionary.
...
in running text. The results of our experiment are promising, but many questions 
remain unanswered.
...
uses a higher proportion of borrowed words, calques, and neologisms with simple
stress patterns.
