...
Russian word stress is variable, which means that the position of the stress in a word is not 
...
spoken Russian, but standard written Russian does not typically mark word stress. Students
of Russian frequently struggle to know how to pronounce written words, so many
textbooks mark stress. Even texts intended for native speakers
sometimes mark words that cannot be disambiguated through context.

Word stress ambiguity in Russian arises both between lexemes and within inflectional
paradigms. Stress ambiguity between lexemes includes pairs of the type
????? \emph{zámok} `castle' and ????? \emph{zamók} 'lock'. For every inflected form of 
these two words, they are both morphosyntactically and orthographically identical, their
meaning differentiated only by stress placement. This kind of ambiguity is relatively uncommon
in Russian. This paper focuses on stress ambiguities which are connected to morphosyntactic
differences. ADD EXAMPLE HERE with ????????, ????????, ????????, AND ??????/??????.
ADD ANOTHER EXAMPLE WITH the paradigms of ???? and ??????.
...
of Czech and French sometimes type emails and other communications
...
it may be more precise to refer to this task as `diacritic enhancement', since it produces
...
to Nodalida-2015 or accepted for publication in its proceedings. 
All submissions must conform to the NoDaLiDa 2015 style guides, as summarized
...
(recommended), or MS~Word. If for some reason you have a problem producing this format, please contact the
Programme Chair (Beata Megyesi; \texttt{beata.megyesi@lingfil.uu.se}) as early as
possible. The NoDaLiDa Programme Committee reserves the right not to publish papers not
