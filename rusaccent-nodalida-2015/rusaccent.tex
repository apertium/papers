...
can render speech incomprehensible. However, standard written Russian does not 
...
Resolving this ambiguity would require the use of  Word Sense Disambiguation, which is
...
Different types of stress ambiguity are overcome by different information. 
Detailed part-of-speech tagging can determine the stress of intraparadigmatic 
homographs and morphosyntactically incongruent homographs, whereas word sense 
disambiguation can determine the stress position of morphosyntactically congruent 
homographs. In the case of running text in Russian, most stress placement 
ambiguity is rooted in morphosyntactic ambiguity, so in this study we focus on 
...
So far, the implicit target application of the few studies related to automatic stress placement
...
unknown tokens (i.e. tokens not found in the FST). Empirical research with
...
it is counted as an `abstention'. The sum of the errors and abstentions is 
referred to as `totFail'. The sum of correct attempts and `errors' is referred to 
as `totTry'. Results are given in Table~\ref{tab:results}.
...
\cite{hall_sproat_russian_2013}\rr{need to cite this without parentheses}), we 
calculate that only 29~518 (1.05\%) 
...
those differences. used in would shed light on those interactions as well.

The syllable-guessing algorithm {\small {\tt Syll}} used in this experiment was 
overly simplistic, and it was not surprising that it was only moderately 
successful. More rigorous approaches have been suggested in other studies.
One approach -- exemplified by \newcite{Church-85}, 
\newcite{Williams-87} and \newcite{Xomicevic_2008} -- is to 
attempt to parse the unknown token by matching known prefixes and suffixes.
Another approach -- exemplified by Pearson.ea-00, Webster-04, Dou.ea-09, and 
\newcite{hall_sproat_russian_2013} -- is based on machine-learning. For unknown 
words, \newcite{hall_sproat_russian_2013} achieve an accuracy of 83.9\%.
It should be noted that their model was trained on a full list of Russian words,
which is not representative of the words that would be unknown to a system like
ours. Most of the complicated wordstress patterns are a closed 
class.\footnote{The growing number of masculine nouns with shifting stress are 
the exception to this generalization.} Future machine-learning work on Russian 
wordstress placement in running text should limit the training data to word
classes that are likely to be similar to unknown tokens, such as those with
