...
This paper presents a preliminary constraint grammar for Russian. The principle 
...
Note that the percentages in Table~\ref{table:ambiguity} overlap, i.e. the readings of a given wordform
can have more than one kind of ambiguity. For example, the wordform \emph{zamkov} has the readings given in \ref{ex:zamkov}.
...
Table~\ref{table:ambiguity} above.
...
It is primarily based on Zaliznjak's \emph{Grammatical dictionary of Russian}, 
...
word stress placement (Author, 2015). This task is especially relevant for 
...
so a constraint grammar can potentially resolve most stress ambiguity indirectly. The results of Author 2015 show that our constraint grammar overcomes about 42\% of the ambiguity relevant to stress
...
decreasing recall. Secondly we would like to add syntactic function labelling and dependency analysis. For 
the dependency analysis we would like to reuse the Giellatekno dependency grammar as in \cite{antonsen10}. 
